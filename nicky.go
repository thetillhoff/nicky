...
/// global variables
var (
	global_filepath           string // define global variable for source file path
...
func main() {
	...
	global_filepath, global_schemapath, global_officialschemapath, global_schema_only, global_schemaisjson, global_disable_unfolding, global_debug = read_cli_arguments() // read and parse cli arguments and check for possible errors
	...
	if !global_schema_only { // if not only the schema should be processed, but also the verification should be made
		documentyaml := load_file(global_filepath) // load document.yaml
		debuglog("content of "+global_filepath, documentyaml)
		...
		debuglog("content of jsonified "+global_filepath, documentjson)
	}

	//
	// The yaml-formatted document is now converted to json and verified against the official (core-)json-schema, which means, it contains valid json after conversion.
	...
	if !global_schema_only { // if only schema is provided and processed, don't do the validation
		...
		if json_validationerrors != nil { // if validation is unsuccessful
			...
			debuglog("", global_filepath+" was successfully validated against provided "+global_schemapath)
