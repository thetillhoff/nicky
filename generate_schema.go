...
func generate_folded_schema(source string) string {
	...
	generated_properties := generate_recursive_map("", mapped_source) // start recursion
	mapped_schema["properties"] = generated_properties                // set the map generated by recursion as properties of root-element

	/// the recursion is now finished
	/// the next block adds some basic values to the root element

	mapped_schema["name"] = "schema_generated_by_nicky"               // set name of schema, which is used in the unfolding-process
	mapped_schema["required"] = get_keys_of_map(generated_properties) // set all first level properties as required

	/// the whole map is now finished
	/// the next block converts the map to yaml and finally returns it
	generated_schema, err := yaml.Marshal(&mapped_schema) // store map into yaml
...
func get_keys_of_map(m map[string]interface{}) []string {
	var keys []string
